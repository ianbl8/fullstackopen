...
sequenceDiagram
    ...
    Note right of browser: Browser redirects to reload the page
    ...
    Note right of browser: Browser executes the JS code to fetch the JSON from the server
    browser->>server: GET https://studies.cs.helsinki.fi/exampleapp/data.json
    activate server
    server-->>browser: JSON format notes, now including the new note
    deactivate server
